    ...
    req(url, (err, body) => {
        ...
        Object.keys(data).forEach(k => {
          ...
          for (var i in $(data[k])) {
              // Processing
          // console.log('------------------');
              // console.log($(data[k])[i].attribs.title);
              // console.log($(data[k])[i].attribs.href);
          // if (!$(data[k])[i].attribs === undefined) {
          //   }
          // console.log('------------------');
          }

          // var arr = $.map($(data[k]), function(el) { return el; });
          // console.log(arr);
          // console.log($('#resultTable table tr td a:eq(0)'));
...
scrape("https://hp.leegardens.com.hk/shopping?lang=en-US", {
    // Get the website title (from the top header)
    title: "#resultTable table"
}, (err, data) => {
  mallData = data;
